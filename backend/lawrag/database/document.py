...
class DocumentStore:
    ...
    async def aimport_parsed(self, law_name: str, parsed: dict, law_index_id: UUID) -> ImportResultDict:
    ...
    async def aimport_laws(self, id: UUID | None = None, law_name: str | None = None) -> list[ImportResultDict]:
    ...
    async def aembed_laws(
        ...
        label = str(id) if id else law_name
        total_nodes = 0
        total_chunks = 0
        ...
        while True:
            async with self.__db.asession() as session:
                ...
                if id is not None:
                    stmt = stmt.where(col(LawNode.law_index_id) == id)
                elif law_name is not None:
