...
- **推理**: `vllm[runai,fastsafetensors,tensorizer]` + `vllm-omni`
- **辅助**: `markitdown[all]`、`orjson`、`asyncer`（`runnify` 把同步 typer 命令转异步）、`cupy-cuda13x`、`onnxruntime-gpu`、`nixl[cu13]`
...
`backend/lawrag/chat/chat_model.py` 默认 `CHAT_MODEL = "qwen3.5"`；`backend/lawrag/documents/embedder.py` 使用 `qwen3-embedding` / `qwen3-reranker`。`model_launch.json` 中的 `model_uid` 必须一致：
...
> `backend/` 用的是 `LLM_PROTOCOL/LLM_HOST/LLM_PORT/LLM_LINK`，`llmserver/` 用的是 `LLM_HOST/LLM_PORT`。两者在 `.env` 同时设置时需保持端口一致，`backend` 才能把请求打到本服务 `/v1` 端点。
...
- 启用的 ruff 规则集：`F/E/W/I/N/FAST/PL/UP/NPY/PD/ASYNC/B/C4/FURB/PTH`（preview），忽略 `PLR*/PLC0415/PLW*/N801/PLR2004`。
