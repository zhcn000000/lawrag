...
| `/chat`     | `ChatPage`          | 是   | 流式 Agent 多轮对话 (顶部 ChatPage 入口)       |
...
`ProtectedRoute` 读取 `localStorage.token` 决定是否放行；`api/eval.ts` 走 `EventSource` / `fetch` 流式订阅。
...
Vite 通过 `vite.config.ts` 从仓库根 `.env` 读取（与 `lawrag/environments.py` 一致）：
